...
public class AppFrame extends JFrame implements ActionListener {
	...
	public AppFrame() {
		...
		importSurvey = new JButton("Import Survey");				// creates a button for importing file
		importSurvey.setFont(new Font("Arial", Font.PLAIN, 11));
		importSurvey.addActionListener(this);
		viewTable = new JButton("View table");						// creates a button for viewing database
		...
		ViewStudentsPanel students = new ViewStudentsPanel();		// creates new VievStudentsPanel
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(importSurvey)) {					// if importSurvey button pressed
			cardPanel.setVisible(true);
			cardLayout.show(cardPanel, "survey");					// show ImportPanel
		} else if (e.getSource().equals(viewTable)) {				// if importSurvey button pressed
			cardPanel.setVisible(true);

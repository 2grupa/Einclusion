...
/**
 * Panel for a table view of Database table with calculated student e-inclusion
 * degrees.
 */
public class ViewStudentsPanel extends JPanel implements ActionListener {
	 ...
	 * 
	 * @param ID
	 *            — identifier (phone number) of the Student
	 * @param name
	 *            — name of the Student
	 * @param course
	 *            - name of the Topic the survey was performed on
	 * @param motivation
	 *            - coefficient value of SWL (students willingness to learn)
	 * @param digSkills
	 *            - coefficient value of DS (digital skills)
	 * @param eRsrce
	 *            - coefficient value of ELM (e-learning materials)
	 * @param eEnvrnmnt
	 *            - coefficient value of ELE (e-learning environment)
	 * @param instructor
	 *            - coefficient value of IWS (instructors willingness to share
	 *            knowledge)
	 * @param learningCpcty
	 *            - coefficient value of SAL (students ability to learn)
	 * @param date
	 *            - date of when the survey was performed
	 * @param M2risk
	 *            - value of students e-inclusion risk degree calculated using
	 *            regression method #2
	 */
	void addTableLine(String ID, String name, String course, String motivation,
			String digSkills, String eRsrce, String eEnvrnmnt,
			String instructor, String learningCpcty, String date, String M2risk) {
	...
	/**
	 * Sets up table and initial filter field values. Prepares table model and
	 * sets column header.
	 */
	void prepareTable() {
	...
	/**
	 * Generates items for comboBox_2 by adding unique values selected column
	 * (selected item in comboBox_1)
	 */
		...
		try {
			...
			if (colName.equals("*") || value.equals("All")) {
				sql = "SELECT * FROM " + DB_TABLE_NAME
						+ " WHERE NAME IS NOT 'test'" + " ORDER BY NAME";
				pStmt = conn.prepareStatement(sql);
			} else if (colName.equals("M2")
					&& (value.equals("Green") || value.equals("Orange") || value
							.equals("Red"))) {
				sql = "SELECT * FROM " + DB_TABLE_NAME
			...
			while (rs.next()) {
				...
				for (RegressionModel rm : regressionModels) {
					if (rm.key.equals(regressionModel)) {
						...
						if (Double.parseDouble(m2risk) == 0)
							m2risk = RegressionModel.getM2regressionDegree(rm,
									Double.parseDouble(mot),
									Double.parseDouble(learnCpcty),
									Double.parseDouble(eRsrc),
									Double.parseDouble(instr),
									Double.parseDouble(eEnvrn))
									+ "";
				...
				addTableLine(ID, name, course, mot, digSkills, eRsrc, eEnvrn,
						instr, learnCpcty, date, m2risk);
			}
		} catch (SQLException se) { // Handle errors for JDBC
			se.printStackTrace();
		} catch (Exception e) { // Handle errors for Class.forName
	...
	/**
	 * Returns short form (abbreviation) of a column header to match with its
	...
	void prepareRegressionModels() {
		try {
			...
			while (rs.next()) {
				regressionModels.add(new RegressionModel(rs.getString("key"),
						rs.getString("value")));
			}
		} catch (SQLException se) { // Handle errors for JDBC
			se.printStackTrace();
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource().equals(comboBox_1)) {
			// Generates combobox_2 values; checks unique values in table where:
			// column = combobox_1.selectedItem
			comboBox2Generate();
		} else if (e.getSource().equals(btnApply)) {
			// makes the table show only filter appropriate items.
			readDBfiltered(comboBox_1.getSelectedItem().toString(), comboBox_2
					.getSelectedItem().toString());
		}
	}

	/**
	 * Creates custom CellRenderer class which changes cell background color for
	 * column 'M2'. Colors are: GREEN, ORANGE or RED, respectively 100-61,
	 * 60-26, 25-0% value in column M2. Compatible with column sorting.
	 * 
	 * @author student
	 */
	@SuppressWarnings("serial")
	private class MyRenderer extends DefaultTableCellRenderer {
		...
		public Component getTableCellRendererComponent(JTable table,
			...
			if (column == 10) {
				...
				double columnValue = Double.parseDouble((String) model
						.getValueAt(modelRow, column)) / 5d;

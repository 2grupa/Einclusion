		...
		try {
			...
			while ((line = br.readLine()) != null) {
				...
				if (5 - Float.parseFloat(value[13]) == 0) {
					...
					SAL = (Float.parseFloat(value[12]) - Float.parseFloat(value[13])) * 4 / (5 - Float.parseFloat(value[13]));
				}
				SAL++;
				System.out.println("this far");
				q = entityManager.createNativeQuery("INSERT into Student (Phone,Name,Topic,IWS,KLAL,KLBL,PU,SubmitDate,SWL,DS,ELM,ELE,SAL,PUOU,M2)"
					+ "SELECT '" + value[0] + "','" + value[1] + "','" + value[2] + "','" + value[11] + "','" + value[12] + "','"
					+ value[13] + "','" + value[14] + "','" + value[15] + "','" + SWL + "','" + DS + "','" + ELM + "','"
					+ ELE + "','" + SAL + "', 0,0 WHERE NOT EXISTS (SELECT TOPIC,PHONE FROM STUDENT WHERE PHONE='" + value[0]
					+ "' " + "AND TOPIC=" + " '" + value[2] + "')");
				q.executeUpdate();
			}
			br.close();

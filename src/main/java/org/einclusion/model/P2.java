...
public class P2 {
  static final Logger LOG = Logger.getLogger(P2.class);
  ...
  static void getPrognosis() {
    try {
      LOG.info("P2 prognosis started");
      Regression regression = (Regression) ModelManager.getObjectValue(
          REGRESSION_KEY, Regression.class);
      String formula = regression.toFormula();
      LOG.info("P2 Regression formula: " + formula);
      ...
      for (Assessment a : assessments) {
        ...
        Query updateQuery = entityManager
            .createQuery("Update Assessment a set a.prog2 =(" + formula
                + ") where a.id=" + a.id);
        updateQuery.executeUpdate();
        transaction.commit();
      }
      LOG.info("P2 prognosis finished successfully");
    } catch (Exception e) {
